...
def insertOrUpdate(Id, Name, roll, batch) :                                            # this function is for database
    ...
    if isRecordExist == 1:                                                      # updating name and roll no
        ...
        connect.execute("UPDATE Students SET Batch = ? WHERE ID = ?",(batch, Id))
    else:
        params = (Id, Name, roll, batch)                                               # insering a new student data
        connect.execute("INSERT INTO Students VALUES(?, ?, ?, ?)", params)
    connect.commit()                                                            # commiting into the database
    connect.close()                                                             # closing the connection

def create(id,stuname,rollno,batch):
    cap = cv2.VideoCapture(0)
    detector = dlib.get_frontal_face_detector()
    Id = id
    insertOrUpdate(id, stuname, rollno, batch)                                                  # calling the sqlite3 database
    ...
    while True:
        ...
        #cv2.waitKey(10)
        if sampleNum >= 80:                                                        # will take 80 faces
            break
